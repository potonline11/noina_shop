...
- หากลูกค้าสนใจ ให้ชวนสมัครสมาชิกเพื่อจับจองพื้นที่ต้นสายและแชร์ลิงก์สร้างรายได้แบบ Passive Income!`;
  ...
  // Recursive helper to follow redirects manually for POST requests (converts POST 302 redirects to GET without body, which Google Apps Script echo requires)
  async function fetchWithPostRedirect(url: string, options: any): Promise<Response> {
    ...
    if ([301, 302, 303, 307, 308].includes(response.status)) {
      ...
      if (redirectUrl) {
        console.log(`[Webhook Proxy Redirect] HTTP ${response.status} redirect detected to: ${redirectUrl}`);
        
        // For 301, 302, 303 redirects, convert method to GET and remove body
        if ([301, 302, 303].includes(response.status)) {
          const newOptions = { ...options };
          newOptions.method = 'GET';
          delete newOptions.body;
          if (newOptions.headers) {
            const cleanHeaders = { ...newOptions.headers };
            delete cleanHeaders['Content-Type'];
            delete cleanHeaders['content-type'];
            newOptions.headers = cleanHeaders;
          }
          return fetchWithPostRedirect(redirectUrl, newOptions);
        } else {
          // For 307 and 308 redirects, preserve POST method and body
  ...
  app.post('/api/webhook-proxy', async (req, res) => {
    try {
      ...
      if (bodyWebhookUrl && sanitizeWebhookUrl(bodyWebhookUrl) !== store.webhookUrl) {
        store.webhookUrl = sanitizeWebhookUrl(bodyWebhookUrl);
        await writeStore(store);
        console.log(`[Webhook Proxy] Automatically updated server-side webhook URL to: ${store.webhookUrl}`);
      }
      
      if (!webhookUrl || !webhookUrl.startsWith('http')) {
        console.warn('Webhook proxy: No webhook URL set in products_store.json on the server or provided in request.');
        return res.json({ 
          success: false, 
      ...
      // Check if response text is HTML (e.g. Google error pages or login screens)
      const isHtml = responseText.trim().toLowerCase().startsWith('<!doctype') || responseText.trim().toLowerCase().startsWith('<html');
      
      if (isHtml) {
        console.warn('[Webhook Proxy] Google Apps Script returned HTML instead of JSON. This usually indicates an incorrect script URL, execution permissions (not set to Anyone), or incorrect deployment setup.');
        return res.json({
          success: false,
          status: response.status,
          message: 'สคริปต์ Google Apps Script ตอบกลับเป็นหน้าเว็บ HTML แทนที่จะเป็นข้อมูลสำเร็จ! โปรดตรวจสอบว่าการตั้งค่า Deployment ใน Google Sheet ถูกตั้งค่าเป็น "Anyone" (ทุกคน) และบันทึก URL ตรงตามในช่องแล้วหรือยัง',
          data: responseText
        });
      }
      ...
      return res.json({
        success: isSuccess,
        status: response.status,
        message: parsedData?.message || 'ส่งข้อมูลไปยัง Google Sheet และสั่งส่งเมล์แจ้งเตือนสำเร็จ!',
