...
- หากลูกค้าสนใจ ให้ชวนสมัครสมาชิกเพื่อจับจองพื้นที่ต้นสายและแชร์ลิงก์สร้างรายได้แบบ Passive Income!`;
  ...
  // Unified registration endpoint: Records member in Google Sheets & sends welcome email via SMTP
  app.post('/api/register', async (req, res) => {
    try {
      ...
      if (isOrder) {
        ...
        if (!finalOrderId || !finalCustomerName || !email) {
          return res.status(400).json({
            success: false,
            message: '❌ ข้อมูลไม่ครบถ้วน: กรุณาระบุรหัสสั่งซื้อ ชื่อ และอีเมลของลูกค้า'
          });
        }

        const rawWebhookUrl = webhookUrl || store.webhookUrl;
        const cleanWebhookUrl = sanitizeWebhookUrl(rawWebhookUrl);
        const finalSheetId = sheetId || (store.sheetUrl ? store.sheetUrl.match(/\/d\/([a-zA-Z0-9_\-]+)/)?.[1] : '');

        let sheetSaved = false;
        let sheetMessage = '';

        // 1. Forward order to Google Sheets
        if (cleanWebhookUrl && cleanWebhookUrl.startsWith('http')) {
          try {
            console.log(`[Order Service] Forwarding order ${finalOrderId} to Google Sheets Webhook...`);
            const trace: any[] = [];
            const sheetResponse = await fetchWithRedirects(cleanWebhookUrl, {
              method: 'POST',
              headers: {
            ...
            console.log(`[Order Service] Google Sheets responded with Status: ${sheetResponse.status}`, responseText);
            ...
            if (sheetResponse.ok && !responseText.includes('<!DOCTYPE') && !responseText.includes('<html')) {
              try {
                ...
                sheetMessage = parsed.message || 'บันทึกออเดอร์ลง Google Sheet สำเร็จ';
              } catch (e) {
                sheetSaved = true;
                sheetMessage = 'บันทึกออเดอร์ลง Google Sheet เรียบร้อยแล้ว (การตอบกลับไม่ใช่ JSON)';
              }
            } else {
              sheetSaved = true;
              sheetMessage = 'ส่งข้อมูลออเดอร์ไปสคริปต์ Google Sheet สำเร็จ แต่ Google ตอบกลับมาเป็นหน้าเว็บ HTML';
            }
          } catch (sheetErr: any) {
            console.error('[Order Service] Google Sheet forward failed:', sheetErr);
            sheetMessage = `ไม่สามารถเชื่อมต่อกับ Google Sheet: ${sheetErr.message}`;
          }
        } else {
          sheetMessage = '⚠️ ไม่พบลิงก์ Google Sheets Webhook (ข้ามขั้นตอนการบันทึกในสเปรดชีต)';
        }

        // 2. Send Invoice Email via SMTP
        ...
        if (!smtpUser || !smtpPass) {
          console.warn('[Order Service] SMTP credentials (SMTP_USER/SMTP_PASS) are not configured.');
          return res.json({
            success: true,
            message: '✅ บันทึกข้อมูลออเดอร์เรียบร้อยแล้ว (ระบบข้ามการส่งอีเมลใบเสร็จเนื่องจากยังไม่ได้ป้อน SMTP_USER หรือ SMTP_PASS)',
            googleSheet: { saved: sheetSaved, message: sheetMessage },
            email: { sent: false, message: 'SMTP credentials missing' }
          });
        }
        ...
        try {
          ...
          const itemsHtml = items ? items.split(', ').map(item => `<li>${item}</li>`).join('') : '';
          ...
          const htmlContent = `
            ...
            <head>
              <meta charset="utf-8">
              <title>ใบเสร็จและการยืนยันคำสั่งซื้อ - Noina Shop</title>
              <style>
                body {
            ...
            <body>
              <div class="container">
                <div class="header">
                  <h1>NOINASHOP - ยืนยันคำสั่งซื้อ</h1>
                  <p>ขอบพระคุณที่อุดหนุนสินค้าไอทีพรีเมียมจากร้าน Noina Shop</p>
                </div>
                <div class="content">
                  <div class="invoice-title">ใบยืนยันคำสั่งซื้อออเดอร์ #${finalOrderId}</div>
                  <p>สวัสดีคุณ <strong>${finalCustomerName}</strong>,</p>
                  <p>เราได้รับการสั่งซื้อของคุณเป็นที่เรียบร้อยแล้ว รายละเอียดและข้อมูลการจัดส่งสินค้ามีดังนี้:</p>
                  
                  <div class="details-card">
                    <div class="details-row">
                      <span class="label">รหัสการสั่งซื้อ:</span>
                      <span class="value highlight">${finalOrderId}</span>
                    </div>
                    <div class="details-row">
                    ...
                    <div class="details-row">
                      ...
                      <span class="value">${paymentMethod === 'cod' ? 'เก็บเงินปลายทาง (COD)' : 'โอนเงินสด'}</span>
                    </div>
                    <div class="details-row">
                      <span class="label">รหัสผู้แนะนำ MLM:</span>
                      <span class="value">${sponsorCode || 'N/A'}</span>
                    </div>
                  </div>
                  ...
                  <div class="items-list">
                    <ul>
                      ${itemsHtml || `<li>${items}</li>`}
                    </ul>
                  </div>

                  <div class="details-card" style="background-color: #ecfdf5; border-color: #a7f3d0;">
                    <div class="details-row">
                      <span class="label" style="color: #047857;">คะแนนสะสมที่ได้รับ:</span>
                      <span class="value" style="color: #047857; font-weight: 800;">+${totalBV || 0} BV</span>
                    </div>
                    <div class="details-row">
                      <span class="label" style="color: #047857;">ยอดรวมชำระสุทธิ:</span>
                      <span class="value" style="color: #047857; font-weight: 800; font-size: 16px;">${totalAmount ? Number(totalAmount).toLocaleString() : '0'} ฿</span>
                    </div>
                  </div>

                  <p><strong>ที่อยู่จัดส่งสินค้า:</strong><br>${address || 'รับหน้าร้าน'}</p>

                  <div class="button-container">
                    <a href="https://noina-shop.vercel.app/" class="btn" target="_blank">
                      ตรวจสอบออเดอร์ของฉัน
                    </a>
                  </div>
                  
                  <p style="font-size: 12px; color: #94a3b8; text-align: center; margin-top: 25px;">
                    หากต้องการเปลี่ยนแปลงข้อมูลการจัดส่ง สามารถติดต่อแอดมิน คุณไวพจน์ โสมภา <strong>081-160-1092</strong> ได้โดยตรง
                  </p>
                </div>
                <div class="footer">
                  <p>© 2026 Noina Shop. All rights reserved.</p>
                  <p>อีเมลฉบับนี้ส่งโดยระบบอัตโนมัติของ <a href="https://noina-shop.vercel.app" target="_blank">Noina Shop</a></p>
                </div>
              </div>
            </body>
            </html>
          ...
          const mailOptions = {
            from: `"Noina Shop" <${smtpUser}>`,
            to: email,
            subject: `ยืนยันการสั่งซื้อออเดอร์ #${finalOrderId} จากร้าน Noina Shop`,
            html: htmlContent
          };

          console.log(`[Order Service] Sending Invoice Email to ${email}...`);
          const info = await transporter.sendMail(mailOptions);
          console.log('[Order Service] Invoice Email sent successfully:', info.messageId);

          return res.json({
            success: true,
            message: '✅ บันทึกข้อมูลและส่งอีเมลใบเสร็จการสั่งซื้อเรียบร้อยแล้ว!',
            googleSheet: { saved: sheetSaved, message: sheetMessage },
            email: { sent: true, messageId: info.messageId }
          });
        } catch (emailErr: any) {
          console.error('[Order Service] SMTP Send Error:', emailErr);
          return res.json({
            success: true,
            message: `✅ บันทึกข้อมูลออเดอร์สำเร็จแล้ว แต่การส่งอีเมลใบเสร็จขัดข้อง: ${emailErr.message}`,
            googleSheet: { saved: sheetSaved, message: sheetMessage },
      ...
      // 1. Forward the registration payload to Google Sheets Web App
      if (cleanWebhookUrl && cleanWebhookUrl.startsWith('http')) {
        try {
          console.log(`[Register Service] Forwarding member ${id} to Google Sheets Webhook...`);
          const trace: any[] = [];
          const sheetResponse = await fetchWithRedirects(cleanWebhookUrl, {
            method: 'POST',
            headers: {
          ...
          const responseText = await sheetResponse.text();
          console.log(`[Register Service] Google Sheets responded with Status: ${sheetResponse.status}`, responseText);
          ...
          if (sheetResponse.ok && !responseText.includes('<!DOCTYPE') && !responseText.includes('<html')) {
            ...
            sheetSaved = true; // Mark as saved to let SMTP proceed
            sheetMessage = 'ส่งข้อมูลไปสคริปต์ Google Sheet สำเร็จ แต่ Google ตอบกลับมาเป็นหน้าเว็บ HTML';
          }
        } catch (sheetErr: any) {
          console.error('[Register Service] Google Sheet forward failed:', sheetErr);
          sheetMessage = `ไม่สามารถเชื่อมต่อกับ Google Sheet: ${sheetErr.message}`;
        }
      } else {
        sheetMessage = '⚠️ ไม่พบลิงก์ Google Sheets Webhook (ข้ามขั้นตอนการบันทึกในสเปรดชีต)';
      ...
      if (!smtpUser || !smtpPass) {
        ...
        return res.json({
          success: true,
          message: '✅ บันทึกข้อมูลสมาชิกเรียบร้อยแล้ว (ระบบข้ามการส่งอีเมลเนื่องจากยังไม่ได้ป้อน SMTP_USER หรือ SMTP_PASS ในแผงควบคุม Secrets)',
          googleSheet: { saved: sheetSaved, message: sheetMessage },
          email: { sent: false, message: 'SMTP credentials missing' }
        });
      }
      ...
      try {
        ...
        console.log(`[Register Service] Sending Welcome Email to ${email}...`);
        const info = await transporter.sendMail(mailOptions);
        console.log('[Register Service] Welcome Email sent successfully:', info.messageId);

        return res.json({
          success: true,
          message: '✅ บันทึกข้อมูลและส่งอีเมลยืนยันการสมัครสมาชิกผ่าน Google Workspace สำเร็จเรียบร้อยแล้ว!',
          googleSheet: { saved: sheetSaved, message: sheetMessage },
          email: { sent: true, messageId: info.messageId }
        });
      } catch (emailErr: any) {
        console.error('[Register Service] SMTP Send Error:', emailErr);
        return res.json({
          success: true,
          message: `✅ บันทึกข้อมูลสำเร็จแล้ว แต่การส่งอีเมลต้อนรับขัดข้อง: ${emailErr.message}`,
          googleSheet: { saved: sheetSaved, message: sheetMessage },
          email: { sent: false, error: emailErr.message }
        });
      }
    } catch (error: any) {
      console.error('[Register Service] Handler Error:', error);
      return res.status(500).json({
        success: false,
        message: `❌ เกิดข้อผิดพลาดของเซิร์ฟเวอร์: ${error.message}`
      });
    }
  });
