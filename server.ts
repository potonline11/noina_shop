    ...
    try {
      ...
      if (isAuthError || (isHtml && (responseText.includes('Sign in') || responseText.includes('Accounts')))) {
        ...
        return res.json({
          success: false,
          status: 401,
          message: '🔒 ไม่สามารถเข้าถึงสคริปต์ได้เนื่องจากติดสิทธิความปลอดภัยของ Google: โปรดแก้ไขสิทธิผู้มีสิทธิเข้าถึง Web App ใน Google Apps Script โดยตั้งค่า Who has access เป็น "Anyone" (ทุกคน) แล้วกด Deploy > New deployment ใหม่ จากนั้นนำลิงก์ /exec ใหม่มาบันทึกในระบบหลังบ้านอีกครั้ง'
        });
      }
