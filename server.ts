...
- หากลูกค้าสนใจ ให้ชวนสมัครสมาชิกเพื่อจับจองพื้นที่ต้นสายและแชร์ลิงก์สร้างรายได้แบบ Passive Income!`;
  ...
  app.post('/api/webhook-proxy', async (req, res) => {
    try {
      ...
      const webhookUrl = bodyWebhookUrl || store.webhookUrl;
      
      if (bodyWebhookUrl && bodyWebhookUrl !== store.webhookUrl) {
        store.webhookUrl = bodyWebhookUrl;
        await writeStore(store);
        console.log(`[Webhook Proxy] Automatically updated server-side webhook URL to: ${bodyWebhookUrl}`);
      }
